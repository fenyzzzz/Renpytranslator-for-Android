...
| Engine | Status | Como funciona |
|--------|--------|---------------|
| **Ren'Py** | ✅ Consolidado | Runtime filter + `tl/<lang>/` |
| **RPG Maker MV/MZ** | ✅ Consolidado | Edição de `data/*.json` + `plugins.js` |
| **RPG Maker VX/Ace** | ⚠️ Parcial | Marshal binário (`rubymarshal`) |
| **Godot Engine** | ⚠️ Experimental | Parser PCK + tradução de `.translation` |
...
2. **Adicione crédito (mínimo $2):**
   ...
   - $2 traduz ~50 jogos médios (mais que suficiente)
...
- Instale no Android (permita "Fontes desconhecidas")
